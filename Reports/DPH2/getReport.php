...
// pull the raw “|”-delimited string from your JS
$rawHourString = $_GET['hours'] ?? '';

// build a clean array of ints, 0–23
$hourList = [];
if (strlen($rawHourString)) {
  $hourList = array_filter(
    array_map('intval', explode('|', $rawHourString)),
...
// ** tack on the HOUR filter **
if (!empty($hourList)) {
    if ($whereClause) {
        $whereClause .= " AND HOUR IN (" . implode(',', $hourList) . ")";
    } else {
        $whereClause = "WHERE HOUR IN (" . implode(',', $hourList) . ")";
    }
}
