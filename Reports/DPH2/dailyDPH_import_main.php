...
foreach ($pslKeys as $clusterName) {
    ...
    while ($group = $getStats->fetch_assoc()) {
        ...
        $leadIdsCsv = $typeCode === 'R' ? "select lead_id from asterisk.vicidial_agent_log{$archiveTable} where (lead_id,event_time) in (select lead_id, MAX(event_time) from asterisk.vicidial_agent_log{$archiveTable}
        where lead_id in ($leadIdsCsv)
        and user = $fronter
        and status = 'CCPLED')" : $leadIdsCsv;
        ...
        $rrpm        = $secTime > 0 ? $group['final_dispos'] / $secTime / 60 : 0;
        ...
        $stmt->bind_param(
            ...
            $num_sales, $total_amount, $group['final_dispos'],
