...
$sql = "
    ...
    WHERE a.call_date BETWEEN '$date 00:00:00' AND '$date 23:59:59'
    AND a.user > 0
    GROUP BY 
        a.user, a.campaign_id, b.list_id, b.$departmentKeyField, vl.list_name,
        CASE WHEN b.$leadTypeField LIKE '%R' THEN 'ROUST_R' ELSE b.$leadTypeField END
    ORDER BY a.user;
";

/* user limitations not needed since only VFRs login to pslv
      ((a.user >= 4000 AND a.user < 5000) OR (a.user >= 8000 AND a.user < 9000) OR (a.user >= 6000 AND a.user < 6100))
      AND 
*/
...
foreach ($getStats as $stat) {
    ...
    $agentType = $stat['list_id'] < 200 ? 1 : 2;
