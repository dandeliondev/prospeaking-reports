<?php
/**
 * Local development bootstrap (mounted at /srv/www/php_include.php in Docker).
 */
...
function prospeaking_wants_mock(): bool
{
    ...
    // No .env: allow mock fallback when MySQL is unavailable.
...
function connectToCluster($name, &$clusters)
{
    ...
    try {
        ...
        if (prospeaking_env_file_exists()) {
            ...
            echo '<p>Copy <code>dev/.env.example</code> to <code>dev/.env</code> and set your MySQL credentials,';
            echo ' or set <code>DEV_MOCK=1</code> in <code>dev/.env</code> to use dummy data without MySQL.</p>';
            exit(1);
        }
...
function prospeaking_mock_banner(): string
{
    ...
    return '<div style="display:block;background:#fff3cd;color:#664d03;padding:6px 12px;text-align:center;font-size:13px;border-bottom:1px solid #ffc107">'
        . 'Demo mode — dummy data (no MySQL). Copy <code>dev/.env.example</code> to <code>dev/.env</code> for real DB, or set <code>DEV_MOCK=1</code>.'
        . '</div>';
}
