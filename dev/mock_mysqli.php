...
/**
 * In-memory stand-in for mysqli when MySQL is not configured (UI demo only).
 */
class DevMockMysqli
{
    ...
    public function query(string $sql)
    {
        ...
        if (stripos($sqlNorm, 'SUM(NUM_SALES)') !== false) {
            ...
            if (stripos($sqlNorm, "/ $n as NUM_SALES") !== false || stripos($sqlNorm, '/ ' . $n . ' as') !== false) {
                $avg = [];
                foreach ($totals as $k => $v) {
                    $avg[$k] = is_numeric($v) ? $v / $n : $v;
                }
                $avg['CONV_RATE'] = $totals['CONV_RATE'];
                $avg['CC_PCT'] = $totals['NUM_SALES'] > 0 ? $totals['CCs'] / $totals['NUM_SALES'] : 0;
